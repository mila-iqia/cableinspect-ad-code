...
from .save_normalized_predictions import SaveNormalizedPredictionsCallback
from .save_predictions import SavePredictionsCallback
...
def get_callbacks(config: DictConfig | ListConfig) -> list[Callback]:
    ...
    if "save_predictions" in config.project.keys():
        add_save_prediction_callback(callbacks, config, is_normalized=False)
    ...
    if "normalization_method" in config.model.keys() and not config.model.normalization_method == "none":
        ...
        if "save_predictions" in config.project.keys():
            # Add callback to save normalized scores
            add_save_prediction_callback(callbacks, config, is_normalized=True)
...
def add_save_prediction_callback(
    callbacks: list[Callback], config: DictConfig | ListConfig, is_normalized: bool
) -> None:
    """Add callbacks to save predictions before and after normalization.

    Args:
        callbacks (list[Callback]): Current list of callbacks.
        config (DictConfig | ListConfig): The config object.
        is_normalized (bool): Flag indicating whether to add SaveNormalizedPredictionsCallback.
    """
    save_image_scores = (
        config.project.save_predictions.image_scores
        if "image_scores" in config.project.save_predictions.keys()
        else False
    )
    save_pixel_scores = (
        config.project.save_predictions.pixel_scores
        if "pixel_scores" in config.project.save_predictions.keys()
        else False
    )
    if is_normalized:
        ...
        callbacks.append(SavePredictionsCallback(config.project.path, save_image_scores, save_pixel_scores))
...
def add_visualizer_callback(callbacks: list[Callback], config: DictConfig | ListConfig) -> None:
    ...
    if config.visualization.log_images or config.visualization.save_images or config.visualization.show_images:
        ...
        extra_threshold_points = (
            config.visualization.extra_threshold_points
            if "extra_threshold_points" in config.visualization.keys()
            else False
        )

        visualizer_callbacks = []
        if "metrics" in config.visualization.include:
            visualizer_callbacks.append(MetricVisualizerCallback)

        if "images" in config.visualization.include:
            visualizer_callbacks.append(ImageVisualizerCallback)

        for callback in visualizer_callbacks:
            callbacks.append(
                callback(
                    ...
                    extra_threshold_points=extra_threshold_points,
                )
            )
