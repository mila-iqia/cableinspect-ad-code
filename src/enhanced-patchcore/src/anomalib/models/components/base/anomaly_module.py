...
class AnomalyModule(pl.LightningModule, ABC):
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        ...
        outputs: Tensor | dict[str, Any] = self.validation_step(batch, from_predict_step=True)
    ...
    def _log_metrics(self, step) -> None:
        """Log computed performance metrics."""
        image_metrics = {step + name: metric for name, metric in self.image_metrics.items()}
        if self.pixel_metrics.update_called:
            pixel_metrics = {step + name: metric for name, metric in self.pixel_metrics.items()}
            self.log_dict(pixel_metrics, prog_bar=True)
            self.log_dict(image_metrics, prog_bar=False)
        else:
            self.log_dict(image_metrics, prog_bar=True)

    def _load_normalization_class(self, state_dict: OrderedDict[str, Tensor]) -> None:
        """Assigns the normalization method to use."""
        if "normalization_metrics.image_max" in state_dict.keys():
