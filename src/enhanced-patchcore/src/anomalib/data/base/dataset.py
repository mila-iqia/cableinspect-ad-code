...
class AnomalibDataset(Dataset, ABC):
    ...
    Args:
        ...
        backbone (str | None, optional): Backbone that is used for feature extraction. Defaults to None.
        image_processor (AutoImageProcessor | None, optional): Image processor for ViT models. Defaults to None.
    """
    ...
    def __getitem__(self, index: int) -> dict[str, str | Tensor]:
        ...
        if self.task == TaskType.CLASSIFICATION:
            if self.backbone is not None and self.image_processor is not None:
                if self.backbone in VIT_BACKBONES:
                    item["image"] = self.image_processor(image, return_tensors="pt")["pixel_values"].squeeze()
                elif self.backbone in SIMPLE_BACKBONES:
                    transformed = self.transform(image=image)
                    image = transformed["image"]
                    item["image"] = self.image_processor.process_input(image)
            else:
                transformed = self.transform(image=image)
            ...
            elif label_index == 1 and not mask_path:
                # This is for the HQ Gauges dataset as there is no mask
                # All pixels in the image are considered abnormal
                mask = np.ones(shape=image.shape[:2])
            else:
                if not os.path.exists(mask_path):
                    # Masks do not exist for this
                    mask = np.zeros(shape=image.shape[:2])
                else:
            ...
            if self.backbone is not None and self.image_processor is not None:
                if self.backbone in VIT_BACKBONES:
                    ...
                    item["mask"] = self.image_processor(
                        mask,
                        return_tensors="pt",
